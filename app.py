...
def main():
    ...
    # 侧边栏顶部导航
    with st.sidebar:
        view = st.radio("功能", ["🏔️ 越野赛预测", "🏃 活动下载"], horizontal=True)
    ...
    if view == "🏔️ 越野赛预测":
        ...
        app_activities._init_state()
        app_activities._render_sidebar()
        app_activities._render_main()
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if gpx_file is not None:
            st.session_state.gpx_file_persist = gpx_file
        ...
        if fit_files:
            existing = st.session_state.get("fits_files_persist", [])
            existing_names = {f.name for f in existing}
            new_files = [f for f in fit_files if f.name not in existing_names]
            st.session_state.fits_files_persist = existing + new_files

        # 使用持久化值显示上传状态
        effective_gpx = st.session_state.get("gpx_file_persist")
        effective_fits = st.session_state.get("fits_files_persist", [])

        if effective_gpx:
            c1, c2 = st.columns([4, 1])
            with c1:
                st.success(f"赛道: {effective_gpx.name}")
            with c2:
                if st.button("删除", key="del_gpx"):
                    st.session_state.gpx_file_persist = None
                    st.rerun()
        if effective_fits:
            st.info(f"训练记录: {len(effective_fits)} 个文件")
            for i, f in enumerate(effective_fits):
                c1, c2 = st.columns([4, 1])
                with c1:
                    st.caption(f"- {f.name}")
                with c2:
                    if st.button("删除", key=f"del_fit_{i}"):
                        st.session_state.fits_files_persist.pop(i)
                        st.rerun()
        ...
        if st.button(":rocket: 开始分析", type="primary", use_container_width=True, key="start_button"):
            if not effective_gpx:
                st.error("请上传赛道GPX文件!")
            elif not effective_fits or len(effective_fits) < 3:
                st.warning("建议至少上传3个训练记录文件")
                if not effective_fits:
                    st.error("请上传训练记录!")
                else:
                    start_analysis(effective_gpx, effective_fits, effort_factor)
            else:
                start_analysis(effective_gpx, effective_fits, effort_factor)
...
def convert_to_prediction_result(result: dict, effort_factor: float) -> PredictionResult:
    ...
    return PredictionResult(
        ...
        segments=segments,
        splits=result.get('split_points', []),
...
def render_split_table(prediction: PredictionResult):
    ...
    for i, sp in enumerate(splits):
        if i == 0:
            ...
            data.append({
                '位置': '🏁 ' + sp['name'],
                '本段距离(km)': '-',
                '距离(km)': f"{sp['distance_km']:.1f}",
                '本段爬升(m)': '-',
                '本段下降(m)': '-',
                '本段时间': '-',
                '累计时间': format_time(sp.get('cumulative_time_min', 0)),
                '平均速度': '-'
            })
        else:
            segment_dist = sp.get('segment_dist_km', 0)
            segment_time = sp.get('time_min', 0)
            avg_speed = segment_dist / (segment_time / 60) if segment_time > 0 else 0

            name_display = sp['name']
            if name_display == '终点':
                name_display = '🏃 终点'
            else:
            ...
            data.append({
                ...
                '距离(km)': f"{sp['distance_km']:.1f}",
                '本段爬升(m)': f"{sp.get('ascent_m', 0):.0f}",
                '本段下降(m)': f"{sp.get('descent_m', 0):.0f}",
    ...
    # 显示说明
    has_cp = any(
        sp['name'] not in ('起点', '终点') and not str(sp['name']).endswith('km')
        for sp in splits
    )
    if has_cp:
        cp_count = sum(
            1 for sp in splits
            if sp['name'] not in ('起点', '终点') and not str(sp['name']).endswith('km')
        )
        st.caption(f"📍 按CP点分段 (共{cp_count}个CP点)")
    else:
