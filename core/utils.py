...
class FilterConfig:
    ...
    # GPX 赛道配置 (过滤 DEM/GPS 海拔噪声)
    # 高斯滤波比 Savitzky-Golay 更保守，不会过冲放大虚假爬升
    GPX = {
        'filter_type': 'gaussian',      # 高斯滤波 (避免 SG 过冲)
        'sigma': 3.0,                   # 平滑强度 (~12-15m 地形覆盖)
        'window_size': 7,               # 保留，切回 SG 时用
        'poly_order': 2,                # 保留，切回 SG 时用
...
class ElevationFilter:
    ...
    def smooth(elevations: List[float] or np.ndarray,
        ...
        filter_type = config.get('filter_type', 'savgol')

        if filter_type == 'gaussian':
            from scipy.ndimage import gaussian_filter1d
            sigma = config.get('sigma', 3.0)
            return gaussian_filter1d(elevations, sigma=sigma)
...
def apply_gpx_filter(elevations: List[float],
    ...
    # 直接从平滑后的海拔差累加爬升/下降，不再通过坡度反推
    ele_diffs = np.diff(smoothed_elevations)
    filtered_gain_m = float(np.sum(np.maximum(ele_diffs, 0)))
    filtered_loss_m = float(np.sum(np.maximum(-ele_diffs, 0)))
    ...
    info = {
        ...
        'filtered_loss_m': round(filtered_loss_m, 0),
