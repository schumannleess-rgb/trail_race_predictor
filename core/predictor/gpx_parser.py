...
class GPXRouteParser:
    ...
    def parse_gpx_route(
        ...
        # 总爬升/下降直接从 segments 汇总，确保和分段数据完全一致
        total_gain = sum(s.segment_ascent_m for s in segments)
        total_loss = sum(s.segment_descent_m for s in segments)
        ...
        route_info = {
            ...
            'split_points':           self._build_split_points(points, distances_m, checkpoints, total_dist_km),
        }
    ...
    def _build_split_points(
        ...
        """构建观测点列表：有CP点按CP点，无CP点按5km间隔

        每个观测点区间的距离/爬升/下降直接从原始轨迹点累加，
        不依赖 0.2km 小段的聚合。
        """
        splits = []
        ...
        splits.append({
            'name': '起点',
            'distance_km': 0.0,
            'altitude_m': points[0]['ele'],
            'point_idx': 0,
        })

        if checkpoints:
            # 有 CP 点：找到每个 CP 点在轨迹上的精确位置，按轨迹距离排序
            cp_with_pos = []
            seen_names = set()
            for cp in checkpoints:
                ...
                nearest_idx = min(range(len(points)), key=lambda i: self._haversine(points[i], cp))
                cp_with_pos.append({
                    'name': cp['name'],
                    'distance_km': distances_m[nearest_idx] / 1000.0,
                    'altitude_m': points[nearest_idx]['ele'],
                    'point_idx': nearest_idx,
                })
            cp_with_pos.sort(key=lambda x: x['distance_km'])
            splits.extend(cp_with_pos)
        else:
            # 无 CP 点：按 5km 间隔
            for mark_km in range(5, int(total_dist_km) + 1, 5):
                target_m = mark_km * 1000
                nearest_idx = min(range(len(distances_m)), key=lambda i: abs(distances_m[i] - target_m))
                splits.append({
                    'name': f'{mark_km}km',
                    'distance_km': distances_m[nearest_idx] / 1000.0,
                    'altitude_m': points[nearest_idx]['ele'],
        ...
        # 终点（如果最后一个中间点已经是终点则不再重复添加）
        last_dist = distances_m[-1] / 1000.0
        if not splits or splits[-1]['distance_km'] < last_dist - 0.001:
            splits.append({
                'name': '终点',
                'distance_km': last_dist,
                'altitude_m': points[-1]['ele'],
                'point_idx': len(points) - 1,
            })

        # 计算每个区间的距离/爬升/下降（直接从原始点累加）
        for i in range(1, len(splits)):
            prev_idx = splits[i - 1]['point_idx']
            curr_idx = splits[i]['point_idx']

            segment_dist_km = (distances_m[curr_idx] - distances_m[prev_idx]) / 1000.0

            ascent = 0.0
            descent = 0.0
            for j in range(prev_idx, curr_idx):
                ele_diff = points[j + 1]['ele'] - points[j]['ele']
                if ele_diff > 0:
                    ascent += ele_diff
                else:
                    descent += -ele_diff

            splits[i]['segment_dist_km'] = round(segment_dist_km, 2)
            splits[i]['ascent_m'] = round(ascent)
            splits[i]['descent_m'] = round(descent)
