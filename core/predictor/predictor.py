    ...
    def predict_race(self, gpx_path: str, effort_factor: float = 1.0) -> Dict:
        ...
        for i, seg in enumerate(segments):
            ...
            segment_predictions.append({
                ...
                'end_km':               round(seg.accumulated_distance_km, 2),
        ...
        # 计算 split points 的预测时间（从 0.2km 小段聚合）
        import copy
        split_points = copy.deepcopy(route_info.get('split_points', []))
        for i in range(1, len(split_points)):
            sp_start_km = split_points[i - 1]['distance_km']
            sp_end_km   = split_points[i]['distance_km']
            split_points[i]['time_min'] = 0.0
            for seg_pred in segment_predictions:
                seg_end_km = seg_pred['end_km']
                if sp_start_km < seg_end_km <= sp_end_km:
                    split_points[i]['time_min'] += seg_pred['segment_time_min']

        # 计算累计时间
        cumulative_time = 0.0
        split_points[0]['time_min'] = 0.0
        split_points[0]['cumulative_time_min'] = 0.0
        for sp in split_points[1:]:
            cumulative_time += sp['time_min']
            sp['cumulative_time_min'] = round(cumulative_time, 1)
