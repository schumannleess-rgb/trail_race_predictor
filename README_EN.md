...
Raw GPS/elevation data contains noise. The system uses Gaussian filtering for smoothing (avoiding Savitzky-Golay over-shoot on undulating terrain):
...
├── app.py                    # Streamlit main application (prediction + activity download)
├── app_activities.py         # Activity download view
├── core/
...
│   │   ├── gpx_parser.py    # GPX route parser + Split Points
...
├── login/                    # Garmin Connect login module
├── activities/               # Activity fetch module
...
|---------|------|---------|
| V1.2.4 | 2026-05-10 | Gaussian GPX filtering, SplitPoints pre-calculation, single-app integration, Garmin activity download |
