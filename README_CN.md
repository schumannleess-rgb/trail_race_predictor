...
原始 GPS/海拔数据存在噪声，本系统采用高斯滤波进行平滑处理（避免 Savitzky-Golay 在起伏地形上的过冲放大）：
...
│   │   ├── gpx_parser.py    # GPX 路线解析 + Split Points
│   │   ├── features.py      # 分段特征定义
│   │   └── cli.py           # CLI 入口
│   ├── types.py             # 类型定义
│   └── utils.py             # 滤波工具
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| V1.2.4 | 2026-05-10 | GPX高斯滤波、SplitPoints预计算、单应用整合、Garmin活动下载 |
