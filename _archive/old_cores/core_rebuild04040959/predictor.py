...
class MLRacePredictor:
    ...
    def train_from_files(self, file_paths: List[str]) -> bool:
        ...
        # Calibrate max_grade from FIT files (P99 of grade distribution)
        fit_paths = [str(f) for f in selected_files if f.suffix.lower() == '.fit']
        calibrated_config = FilterConfig.calibrate_from_fit_files(fit_paths)
        original_fit_config = FilterConfig.FIT.copy()
        FilterConfig.FIT = calibrated_config
        print(f"    Calibrated max_grade_pct: {calibrated_config['max_grade_pct']:.0f}% (default was {original_fit_config['max_grade_pct']:.0f}%)")
        ...
        try:
            ...
            for file_path in selected_files:
                try:
                    if file_path.suffix.lower() == '.fit':
                        segments, rest_ratio = FeatureExtractor.extract_from_fit(file_path)
                    else:  # JSON
            ...
            # Restore original FIT config (undo calibration mutation)
        ...
        if self.predictor.train(all_segments):
            ...
            self.training_stats = {
                ...
                'calibrated_max_grade_pct': calibrated_config['max_grade_pct']
            }
    ...
    def predict_race(self, gpx_path: str, effort_factor: float = 1.0) -> Dict:
        ...
        # Apply rest correction: moving time -> total time (including rest stops)
        moving_time_min = total_time_min
        ...
        return {
            ...
            'predicted_speed_kmh': round(total_distance / (total_time_min / 60), 2) if total_time_min > 0 else 0,
